...
import { ArrowLeft, Edit, AlertCircle, FileText, Activity } from 'lucide-react';
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex border-b border-slate-200">
        ...
        >Assessments ({studentSessions.length})</button>
        <button 
      ...
      {activeTab === 'overview' && (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <div className="card lg:col-span-1 space-y-4">
            <div className="flex justify-between items-start">
              <div>
                ...
                <p className="text-sm text-slate-500 mt-1">Grade {student.grade} • Class {student.classGroup}</p>
              </div>
            ...
            <div className="border-t border-slate-100 pt-4 mt-4 space-y-3 text-sm">
               <div className="flex justify-between"><span className="text-slate-500">Status</span> <span className="font-medium text-slate-800">{student.status}</span></div>
               <div className="flex justify-between"><span className="text-slate-500">DOB</span> <span className="font-medium text-slate-800">{new Date(student.dateOfBirth).toLocaleDateString()}</span></div>
               <div className="flex justify-between"><span className="text-slate-500">Gender</span> <span className="font-medium text-slate-800">{student.gender}</span></div>
               <div className="flex justify-between"><span className="text-slate-500">Language</span> <span className="font-medium text-slate-800">{student.languageOfInstruction || 'N/A'}</span></div>
               <div className="flex justify-between"><span className="text-slate-500">Consent</span> <span className="font-medium text-slate-800">{student.parentalConsentGiven ? 'Yes' : 'No'}</span></div>
            </div>
          ...
          <div className="lg:col-span-2 space-y-6">
            <div className="card">
              <h3 className="text-lg font-semibold text-slate-800 mb-4 flex items-center gap-2"><Activity className="w-5 h-5 text-indigo-500" /> DRI Progress</h3>
            ...
            <div className="card">
              <h3 className="text-lg font-semibold text-slate-800 mb-4 flex items-center gap-2"><Radar className="w-5 h-5 text-indigo-500" /> Latest Domain Profile</h3>
              {latestSession ? (
                <div className="h-72">
                  <ResponsiveContainer width="100%" height="100%">
          ...
          <div className="overflow-x-auto">
             <table className="w-full text-left border-collapse">
               ...
               <tbody className="text-sm">
                 ...
                 {studentSessions.length === 0 && (
                   <tr><td colSpan={5} className="py-8 text-center text-slate-500">No assessments on record.</td></tr>
                 )}
               </tbody>
             </table>
          </div>
